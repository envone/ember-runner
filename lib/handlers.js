...
handlersManager.add("removeRequires", function(context, callback) {
  var regexp = new RegExp("^require\\([\"']([a-zA-Z_\\-\\/]*)[\"']\\)\\;", "gm"),
...
var encloseExportFunctionRegex = new RegExp("packages\/loader", "g");

handlersManager.add("encloseExportFunction", function(context, callback) {
  //console.log(context.file.path);
  //console.log(encloseExportFunctionRegex.exec(context.file.path));
  if (encloseExportFunctionRegex.exec(context.file.path)) {
...
handlersManager.add("includeTemplate", function(context, callback) {
  ...
  context.buffer = "\nEmber.TEMPLATES['" + context.file.templateName(context.templatePath) + "'] = Ember.Handlebars.compile('" + context.buffer + "');\n";
  callback(null, context);
});
...
handlersManager.add("includeCompiledTemplate", function(context, callback) {
  ...
  context.buffer = "\nEmber.TEMPLATES['" + context.file.templateName(context.templatePath) + "'] = Handlebars.template(" + sandBoxContext.templatejs + ");\n";
...
handlersManager.add("scss", function(context, callback) {
  sass.render(context.buffer, function(err, css) {
    if(err) {
	  context.buffer = "\n/* [SASS: ERROR] " + context.file.name + context.file.extname + " not generated, because: \n" + err + "*\\/";
...
handlersManager.add("styl", function(context, callback) {  
  stylus(context.buffer).use(nib()).render(function(err, css) {
    if (err) {
      context.buffer = "\\/* [STYLUS: ERROR] " + context.file.name + context.file.extname + " not generated, cause: \n" + err + "*\\/";
      console.log(context.buffer);
    } else {
      context.buffer = css;
    }
...
handlersManager.add("uglify", function(context, callback) {  
  var ast = jsp.parse(context.buffer); // parse code and get the initial AST

  ast = pro.ast_mangle(ast); // get a new AST with mangled names
  ast = pro.ast_squeeze(ast); // get an AST with compression optimizations

  context.buffer = pro.gen_code(ast); // compressed code here
